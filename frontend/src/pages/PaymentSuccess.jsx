...
const PaymentSuccess = () => {
  // Backend already verified the payment and updated the DB before redirecting here.
  // We just decode the data param for display purposes.
  ...
  if (data) {
    try {
      ...
      finalAmount   = decoded?.total_amount;
      txCode        = decoded?.transaction_code;
    } catch { /* ignore */ }
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-red-50 to-orange-100 flex items-center justify-center p-4">
      <div className="bg-white p-10 rounded-2xl shadow-2xl text-center max-w-md w-full border-4 border-yellow-400 relative overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-orange-50 to-red-50 p-4 rounded-xl mb-6 border-2 border-yellow-400">
          {finalAmount && <p className="text-red-800 font-bold text-xl mb-1">Total: Rs. {finalAmount}</p>}
          {txCode      && <p className="text-xs text-red-500 font-mono">Ref: {txCode}</p>}
